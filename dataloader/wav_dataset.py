...
class WAVDataset(Dataset):
    ...
    def __init__(self, dir: Path, n_fft, test=False):
        ...
        self.clean_WAVs = {}
        for i, file_path in enumerate(sorted(self.GetPaths(self.clean_dir))):
            self.clean_WAVs[i] = file_path
        # for i, filename in enumerate(sorted(os.listdir(self.clean_dir))):
        #     self.clean_WAVs[i] = self.clean_dir.joinpath(filename)

        self.noisy_WAVs = {}
        for i, file_path in enumerate(sorted(self.GetPaths(self.noisy_dir))):
            self.noisy_WAVs[i] = file_path
        ...
        self.crop = RandomCrop((2, 160000), pad_if_needed=True)

    def GetPaths(self, direc):

        folders = os.listdir(direc)
        folders = [os.path.join(direc, x) for x in folders]
        files_path = [[os.path.join(one_folder, file_name) for file_name in os.listdir(one_folder)]\
    ...
    def __getitem__(self, idx):
        ...
        clean_path = self.clean_WAVs[idx]
        ...
        # sleep(100)
        # noisy_path = '20-205-0000_noisy.wav'
        # clean_path = '20-205-0000_noisy.wav'
        while True:
            try:
        ...
        # For sizes of tensors of batch to be matched
        if not self.test:
            sounds = self.crop(torch.cat([clean_waveform, noisy_waveform], dim=0))
            clean_waveform, noisy_waveform = sounds[:1], sounds[1:]
        else:
            # print(clean_path)
            pass
        # print(clean_waveform.shape, noisy_waveform.shape)
        # from time import sleep
        # sleep(5)
